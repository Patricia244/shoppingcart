...
function getCustomerBasket(email){
   return  myArray.filter(element =>element.email === email);
}

getCustomerBasket("tshepo@umuzi.org")
function getAllCustomers(){
    const AllCustomers =  myArray.map(element => element.email)
...
function getPaidItems() {
    return myArray.filter(cart => cart.status.toLowerCase() === "paid")
    .map(cart => cart.items)
}
...
function getConsolidatedItems(allCustomerItems) {
    ...
    for (let customerNitems of allCustomerItems) {
        for (let item of customerNitems) {
            let pos = consolidatedItems.findIndex(cItem => cItem.name === item.name)
...
function totalSpent(email) {
    let customerItems = myArray.filter(element => element.email === email)
        const items = customerItems.filter((cart => cart.status ==='PAID' || cart.status ==='DELIVERED' )).map(cart => cart.items)
        const merged = ([].concat.apply([],items)).map(item => item.price) 
        let total =  merged.length > 0 ? merged.reduce((total, currentValue) => total + currentValue) : 0
    return total 
}
 //console.log(totalSpent( "mo@umuzi.org"))

function topCustomer() {
    let customers = []

    let customerEmails = myArray.map(customer => customer.email).filter((value, index, self) => self.indexOf(value) === index)
    for(let i = 0; i < customerEmails.length;i++) {
        customers.push({email: customerEmails[i], total:totalSpent(customerEmails[i])})
    }
